...
*/
  onCanvasReady() {
    ...
    my.createSelectorQuery().select('#canvasTest').node().exec(async (res) => {
      ...
      drawBackgroundImage({ context, image: backgroundImage,systemInfo,x:0,y:0 });
...
*/
  ...
  createAnimation(timestamp) {
    if (!isDragging) {
      ...
      const { width: canvasWidth, height: canvasHeight } = this.data.canvasConfig;
      // if (( x >=canvasWidth- windowWidth / 2 &&  y >=canvasHeight- windowHeight / 2) && (x <= windowWidth / 2 && y <= windowHeight / 2)) {
      //   //半屏宽高以内 不需要移动canvas 保证安全距离
      //   console.log('半屏宽高以内');
      // }else if (x > windowWidth / 2 && y <= windowHeight / 2) {
      //   // x坐标大于半屏宽度  y 坐标小于半屏高度
      //   context.setTransform(1, 0, 0, 1, windowWidth / 2 - x, 0);
      //   this.setData({
      //     style:{
      //       width:canvasWidth+windowWidth / 2 - x + "px",
      //       height:canvasHeight+'px',
      //     }
      //   })
      // } else if (x < windowWidth / 2 && y > windowHeight / 2) {
      //   // x坐标小于半屏宽度  y 坐标大于半屏高度
      //   context.setTransform(1, 0, 0, 1, x, windowHeight / 2 - y);
      //   this.setData({
      //     style:{
      //       width:canvasWidth+'px',
      //       height:canvasHeight+windowHeight / 2 - y+'px',
      //     }
      //   })
      // } else if (x > windowWidth / 2 && y > windowHeight / 2) {
      //   // x坐标大于半屏宽度  y 坐标小于半屏高度
      ...
      //     }
      //   })
      // }  else if (x <canvasWidth- windowWidth / 2 && y >canvasHeight- windowHeight / 2) {
      //   // x坐标安全距离不足 y坐标安全距离足够
      //   context.setTransform(1, 0, 0, 1,windowWidth / 2- canvasWidth, windowHeight / 2 - y);
      //   this.setData({
      //     // style:{
      //     //   width:canvasWidth+windowWidth / 2 - x+'px',
      //     //   height:canvasHeight+windowHeight / 2 - y+'px',
      //     // }
      //     'style.height':canvasHeight+windowHeight / 2 - y+'px'
      //   })
      // } else if (x >canvasWidth- windowWidth / 2 && y <canvasHeight- windowHeight / 2) {
      //   // y坐标安全距离不足  x坐标安全距离足够
      //   context.setTransform(1, 0, 0, 1, windowWidth / 2 - x, windowHeight/2 - canvasHeight);
      //   this.setData({
      //     // style:{
      //     //   width:canvasWidth+windowWidth / 2 - x+'px',
      //     //   height:canvasHeight+windowHeight / 2 - y+'px',
      //     // }
      //     'style.width':canvasHeight+windowWidth / 2 - x+'px'
      //   })
      // }
      console.log('createAnimation', x, y);
      console.log('progressInSegment', progressInSegment, segment);
      drawBackgroundImage({ context, image: backgroundImage,systemInfo,x,y});
      createRouteLine({ context, points });
      drawCharacterImage({ context, image, tempX: x, tempY: y });
